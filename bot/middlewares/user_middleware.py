    ...
    async def _cleanup_expired(self) -> None:
        ...
        for username, notifications in self._notifications.items():
            valid_notifications = [
                ...
                if current_time - n.timestamp < self._ttl_seconds
            ]
                ...
                if ent.type == MessageEntityType.MENTION:
                    ...
                    if not ref_user:
                        if bot:
                            ...
                            await bot.send_message(
                                db_user.user_id,
                                _("user_mention_not_registered", username=ref_username)
                            )
                        await self._notification_queue.add_notification(
                            ref_username, handler, event, dict(data)
                        )
