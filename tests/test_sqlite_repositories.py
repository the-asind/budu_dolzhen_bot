        ...
        try:
            ...
            if bot.db.connection._pool is not None:
                ...
                try:
                    ...
                    while (not bot.db.connection._pool.empty() and 
                ...
                try:
                    if await asyncio.wait_for(_ensure_database_accessible(db_path), timeout=10.0):
                        ...
                        if access_attempt < max_access_retries - 1:
                            logger.warning(f"Database accessibility check failed, retrying... (attempt {access_attempt + 1})")
                            await asyncio.sleep(0.1 * (2 ** access_attempt))
                        else:
                            raise RuntimeError(f"Cannot access temporary database after {max_access_retries} attempts: {db_path}")
                except asyncio.TimeoutError:
                    if access_attempt < max_access_retries - 1:
                        logger.warning(f"Database accessibility check timed out, retrying... (attempt {access_attempt + 1})")
                        await asyncio.sleep(0.1 * (2 ** access_attempt))
                    else:
                        raise RuntimeError(f"Database accessibility check timed out after {max_access_retries} attempts: {db_path}")
            ...
            for attempt in range(max_file_cleanup_retries):
                try:
                    ...
                    if attempt < max_file_cleanup_retries - 1:
                        ...
                        logger.error(f"Failed to cleanup temporary database file after {max_file_cleanup_retries} attempts: {e}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if await _verify_database_schema(temp_db):
                ...
                import bot.db.connection
                logger.debug(f"Pool state after initialization - initialized: {bot.db.connection._pool_initialized}, pool: {bot.db.connection._pool is not None}")
