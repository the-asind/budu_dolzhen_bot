...
from aiogram.types import Update, MessageEntity
...
from bot.db.repositories import UserRepository, _acquire_connection
...
class TestUnregisteredUserHandling:
    ...
    def registered_user(self):
        ...
        return UserModel(
            user_id=123, username="registered_user", first_name="John", last_name="Doe", language_code="en"
        )

    @pytest.fixture
    def unregistered_telegram_user(self, model_user):
        """Mock unregistered Telegram user."""
        return model_user(id=456, is_bot=False, first_name="Jane", last_name="Smith", username="unregistered_user")
    ...
    def mock_update_with_mention(self, registered_user, unregistered_telegram_user, model_user):
        ...
        message = make_mutable_message(
            ...
            from_user=from_user,
            entities=[MessageEntity(type=MessageEntityType.MENTION, offset=4, length=18)],
        )
    ...
    def mock_start_update(self, unregistered_telegram_user):
        ...
        message = make_mutable_message(text="/start", from_user=unregistered_telegram_user)
...
class TestUnregisteredUserDetection(TestUnregisteredUserHandling):
    ...
    async def test_detect_unregistered_user_by_mention(
        self, user_middleware, mock_user_repo, mock_update_with_mention, registered_user, mock_bot
    ):
    ...
    async def test_detect_unregistered_user_by_text_mention(
        self, user_middleware, mock_user_repo, registered_user, unregistered_telegram_user, mock_bot, model_user
    ):
        ...
        message = make_mutable_message(
            ...
            from_user=from_user,
            entities=[
                MessageEntity(type=MessageEntityType.TEXT_MENTION, offset=4, length=4, user=unregistered_telegram_user)
            ],
        )
    ...
    async def test_no_detection_for_registered_user(
        ...
        another_user = UserModel(
            user_id=456, username="unregistered_user", first_name="Jane", last_name="Smith", language_code="en"
        )
...
class TestDelayedNotificationDelivery(TestUnregisteredUserHandling):
    ...
    async def test_no_delayed_delivery_for_empty_queue(
        self, user_middleware, mock_user_repo, mock_start_update, unregistered_telegram_user, mock_bot
    ):
...
class TestRegistrationEnforcement(TestUnregisteredUserHandling):
    ...
    async def test_block_unregistered_user_access(
        self, user_middleware, mock_user_repo, unregistered_telegram_user, mock_bot
    ):
        ...
        message = make_mutable_message(text="/balance", from_user=unregistered_telegram_user)
    ...
    async def test_allow_registered_user_access(
        self, user_middleware, mock_user_repo, registered_user, mock_bot, model_user
    ):
        ...
        message = make_mutable_message(text="/balance", from_user=from_user)
    ...
    async def test_start_command_bypasses_registration_check(
        ...
        # Execute
        with patch("bot.middlewares.user_middleware.UserRepository", mock_user_repo):
            await user_middleware(handler, mock_start_update, data)
...
class TestNotificationServiceUnregisteredHandling(TestUnregisteredUserHandling):
    ...
    async def test_process_queued_notifications_success(self, notification_service, mock_bot):
        ...
        notification_service.process_queued_notifications = (
            notification_service.__class__.process_queued_notifications.__get__(notification_service)
        )
...
class TestTimeoutHandling(TestUnregisteredUserHandling):
    ...
    async def test_cleanup_expired_pending_notifications(self, user_middleware):
        ...
        if hasattr(UserMiddleware, "_notification_queue") and hasattr(
            UserMiddleware._notification_queue, "add_notification"
        ):
            await UserMiddleware._notification_queue.add_notification("old_user", old_handler, MagicMock(), old_data)
    ...
    async def test_notification_queue_size_limits(self, user_middleware):
        ...
        if hasattr(UserMiddleware, "_notification_queue") and hasattr(
            ...
            for i in range(100):
                handler = AsyncMock()
                data = {"index": i}
                await UserMiddleware._notification_queue.add_notification("spam_user", handler, MagicMock(), data)
    ...
    async def test_cleanup_expired_with_timestamp(self, user_middleware):
        ...
        if hasattr(UserMiddleware, "_notification_queue") and hasattr(
            ...
            for i in range(5):
                handler = AsyncMock()
                data = {"index": i}
                await UserMiddleware._notification_queue.add_notification(f"user_{i}", handler, MagicMock(), data)
    ...
    async def test_enforce_queue_limit_with_custom_size(self, user_middleware):
        ...
        if hasattr(UserMiddleware, "_notification_queue") and hasattr(
            ...
            for i in range(20):
                handler = AsyncMock()
                data = {"index": i}
                await UserMiddleware._notification_queue.add_notification("test_user", handler, MagicMock(), data)
...
class TestRegistrationStateTracking(TestUnregisteredUserHandling):
    ...
    async def test_track_invited_state(self, user_middleware, mock_user_repo, registered_user, mock_bot, model_user):
        ...
        message = make_mutable_message(
            ...
            from_user=from_user,
            entities=[MessageEntity(type=MessageEntityType.MENTION, offset=4, length=13)],
        )
